...
def welcome(name):
    ...
    WHERE USERNAME= '""" +name+ """ '""")
    # for row in cursor:
    #     user.append({"ID": row[0], "username": row[1], "name": row[2], "email": row[3], "phone": row[4], "phong": row[5], "chucvu": row[6], "chuyennganh": row[7], "luongcb": row[8], "hsl": row[9], "luong": int(row[10])})
    rec=cursor.fetchone()
...
def findUser():
    if request.method == "POST": #loi pthuc khong phai Get ma la Post
        ...
        if cursor:
            return render_template("infoFind.html", name=hoten, records=cursor)
